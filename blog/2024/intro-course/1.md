...
- C++のプログラムはすべてmain関数から上から下に順番に実行されます。
...
- 数字で始まる名前
- アルファベット・数字・_(アンダースコア)以外が使われている名前
- 予約語(C++で使われている一部の単語・「using」「int」「namespace」など)
...
int main() {
    ...
    a = 3;
    b = 5;
    int c, d = 8;//宣言と同時に初期化(この場合、初期化されるのはdのみ)
    b = 10;//bの値を10に変更
    cout << a + b + d<< endl;//21と出力される
    cout << c << endl;//何が出力されるかはわからない
}
```
