...
- 要素が複数含まれる場合、`contains`は高速に動作する。

:::details[C++のバージョンについて]
C++にはいくつかバージョンがあり、バージョンによって使える機能が異なる。
C++20以降の機能を使いたい場合は、ローカルのG++の設定を変更し、Atcoderの提出時にバージョンをC++20以上のものを選択する必要がある。
...
※これはC++20を使う為の設定の一例であり実験的な機能であるため、ずっと使える保証はないことに注意が必要。
