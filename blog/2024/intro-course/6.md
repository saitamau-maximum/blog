...
- `priority_queue<型, vector<型>, greater<型>> 変数名;`と書くことで小さい順に取り出されるように宣言される。

### 使い方

| 関数 | 機能 | 計算量 |
| :--- | :--- | :--- |
| `priority_queue<型> pq;` | priority_queue型の変数`pq`を宣言する | $O(logN)$ |
| `pq.push(x);` | priority_queue型の変数`pq`に`x`を追加する | $O(1)$ |
| `pq.top()` | priority_queue型の変数`pq`の中で最大の要素にアクセスする | $O(1)$ |
| `pq.pop()` | priority_queue型の変数`pq`中で最大の要素を削除する | $O(logN)$ |
...
| `d.insert(d.begin()+i,j)` | deque型の変数`d`の`i`番目に`j`を挿入する | $O(1)$ |
...
| `d.clear()` | deque型の変数`d`の中の全ての要素を削除する | $O(1)$ |
...
- `deque`に数値を入れていってソートして前後n個を削除しましょう
- `vector`と同じ形でソートできます
