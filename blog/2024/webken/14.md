...
セッションベース認証は、クライアントにセッションIDを発行しブラウザで保持させます。リソースを要求するときにセッションIDを送信し、サーバー側でセッションIDを検証することで認証を行います。

### トークンベース認証

トークンベース認証は、セッションベース認証で用いていたCookieの代わりに、トークンを利用します。

後ほど詳しく説明します。

### OAuth/OAuth2

OAuthとは、GoogleやFacebook, X(Twitter)などの外部サービスの認証を利用するためのプロトコルです。
