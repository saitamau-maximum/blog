...
ブラウザがwww.example.comにアクセスすると、www.example.comのサーバーにリクエストが送られます。すると、サーバーのデータべースからHTMLファイルやその他(画像、CSS、JavaScriptなど)のファイルを取得し、ブラウザに返して表示します。
...
ポートの転送を停止(解放)するには、`Ctrl + C`(Windows)または`Command + C`(Mac)を押してください。

作成したファイルをGitリポジトリに追加し、コミットしましょう。

```bash
git add . # 全てのファイルを追跡対象とする
...
```bash
node server.js & # &を付けるとバックグラウンドで実行される
```
...
`/users`というURLにアクセスした場合、`userList`という**JSON形式のデータ**が表示(レスポンス)されます。
...
`package.json`が作成されたら、以前と同じように追記してください。
...
今回はNode.jsを使ってWebサーバーを作成し、ルーティングについて学習したり、JSON形式のデータを扱ってみたりしました。今までのJavaScriptの書き方とは慣れない部分が多いと思います。基礎的な文法を押さえたあとは、覚える必要はなくコピペなどで実装しても構いません。次回は、Node.jsより簡潔にコードを書くことができるHonoというフレームワークを使ってTODOアプリを作る予定です。
