...
これからデータベースファイルを作成しますが、データベースに入っているデータにもし個人情報などが含まれていた場合大変ですよね。
そのような場合は、データベースファイルをGitで管理しないようにするために`.gitignore`ファイルを使いましょう。
...
`git push -f`はリモートリポジトリを作成していれば行なってください。
...
筆者(sor4chi)は勝手に名前を変えられない名前付きインポートの方が好きですが、デフォルトインポートの方を使う方も多いのでお好みでどうぞ。
Maximumは名前付きインポート勢が多いです。
...
この話は一応`SQLite3`もとい`SQL`の話なのですが、前回紹介されていなかったのでここで紹介します。
...
とすると、`id`が`2`のデータだけが取得できていればOKです。
...
最後に、要らなくなった`todoList`と`currentId`を削除して終わりです、完成はこうなりました。
...
また、`AUTOINCREMENT`を使うことで`id`を自動で付与できるようになったり、実はデータの検索が$O(1)$になっていたりします。
