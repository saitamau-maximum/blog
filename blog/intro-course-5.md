...
このプログラムでは、for 文を n 回繰り返しているので、計算回数はおおよそ n 回になります。この時時間計算量はオーダー記法を用いて`O(n)`と表します。このように、計算量を表すときには、計算回数ではなく、計算回数のオーダーを表します。
...
このコードは$N$を$2$で割り続けているので、$O(logN)$であらわされます。
計算量の$log$は底はなんでもいいです。今回は、$log_2(N)$になります
