...
$O(1) < O(\mathrm{\log}N) < O(N) < O(N\mathrm{\log}N) < O(N^2) < O(2^N) < O(N!)$
...
これは$O(N\mathrm{\log}N)$です。$\log(N)$を$N$かい繰り返しているので、$N\mathrm{\log}N$になります。
...
boolのAND`&&`,OR`||`との区別を理解しておこう。
特に、bitごとの演算は「bitwise 〇〇」という言い方をする。
...
$1,2,4$の中から自由に数字を選び、選び出した数字の和でいくつの数字を作ることができるか。一つも選ばない場合は0になる。
